...
struct get_impacted_account_visitor
{
   ...
   {
      _impacted.insert( op.to ); // to
      _impacted.insert( op.fee_payer() ); // from
   }
   void operator()( const invoice_create_operation& op )
   {
      _impacted.insert( op.customer ); // customer
      _impacted.insert( op.fee_payer() ); // merchant
   }
   void operator()( const invoice_pay_operation& op )
   {
      _impacted.insert( op.merchant ); // merchant
      _impacted.insert( op.fee_payer() ); // customer
   }
   void operator()( const new_invoice_create_operation& op )
   {
      _impacted.insert( op.merchant ); // merchant
      _impacted.insert( op.customer ); // customer
      _impacted.insert( op.fee_payer() ); // creator
   }
   void operator()( const new_invoice_pay_operation& op )
   {
      _impacted.insert( op.merchant ); // merchant
      _impacted.insert( op.customer ); // customer
      _impacted.insert( op.fee_payer() ); // customer
   }
   void operator()( const account_status_give_operation& op )
   {
   ...
   void operator()( const account_status_give_light_operation& op )
   {
      _impacted.insert( op.account_to_upgrade ); // account_to_upgrade
      _impacted.insert( op.fee_payer() ); // giver
   }
   ...
   if( obj->id.space() == protocol_ids )
   {
      switch( (object_type)obj->id.type() )
      {
        ...
        case invoice_object_type:{
           const auto& aobj = dynamic_cast<const invoice_object*>(obj);
           FC_ASSERT( aobj != nullptr );
           accounts.insert( aobj->customer );
           accounts.insert( aobj->merchant );
           break;
         }
        case new_invoice_object_type:{
           const auto& aobj = dynamic_cast<const new_invoice_object*>(obj);
           FC_ASSERT( aobj != nullptr );
           accounts.insert( aobj->customer );
           accounts.insert( aobj->merchant );
           break;
